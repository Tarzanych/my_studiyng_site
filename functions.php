...
// 					echo "select count(`id`) from `menuItems` where `publish`=1 and `parent`=".$item['id'];
	...
	class User {
		...
		public function CheckPermissions($perm) {
			...
			switch ($perm) {
				case 'create':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['create']>0 || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
				case 'publish':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['publish']>0 || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
				case 'edit':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['edit']>0  || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
				case 'delete':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['delete']>0  || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
				case 'publishOwn':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['publishOwn']>0 || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
				case 'editOwn':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['editOwn']>0  || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
				case 'deleteOwn':
					if ((isset($this->Permissions) && !$this->IsAnonymous && ($this->Permissions['deleteOwn']>0  || $this->Permissions['root']>0 || $this->Permissions['admin']>0 )) && isset($this->IsActive) && $this->IsActive && isset($this->IsBlocked) && !$this->IsBlocked) $return = true;
				break;
			}
			return $return;
		}
...
//  			$urlArray = array_reverse($urlArray);
	...
	function t($argument, $lang=0) {
		global $db;
		if ($lang==0) $lang = intval($_SESSION['language']);
	...
	function checkFilledArray($a) {
		...
		foreach ($a as $key=>$val) {
			if (GetOne("select count(`id`) from `languages` where `id`=".$key) && trim($val)=="") $return = false;
		}
		return $return;
	}
?>