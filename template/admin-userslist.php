...
<table class="usersTable adminTable">
    ...
    foreach ($this->UsersList as $user) {
        ...
        <tr <?= $user['deleted'] == 1 || $user['blocked'] == 1 ? 'class="deleted"' : '' ?>>
            ...
            <td>
                <a href="<? if ($user['deleted'] == 0) { ?><?= $this->RootUrl ?>profile/<?= $user['id'] ?><?
                } else {
                    echo "#";
                }
                ?>"><?= $user['nickname'] ?></a>
            ...
            <td class="center">
                <? if ($user['blocked'] == 0 && $user['deleted'] == 0 && (!$user['isAdmin'] || $User->Permissions['root'] == 1) && $user['id'] != $User->Id) { ?>
                    <a class="red" href="<?= $this->RootUrl ?>admin/userslist/blockprofile/<?= $user['id'] ?><?= isset($_GET) ? "?" . http_build_query($_GET) : "" ?>"  onclick="if (confirm('Do you really want to block profile <?= $user['nickname'] ?>?')) {
                                return true;
                            } else {
                                return false;
                            }">block</a>
            ...
            <td class="center">
                <? if ($user['deleted'] == 0 && (!$user['isAdmin'] || $User->Permissions['root'] == 1) && $user['id'] != $User->Id) { ?>
                    <a class="red" href="<?= $this->RootUrl ?>admin/userslist/delprofile/<?= $user['id'] ?><?= isset($_GET) ? "?" . http_build_query($_GET) : "" ?>" onclick="if (confirm('Do you really want to delete profile <?= $user['nickname'] ?>?')) {
                                return true;
                            } else {
                                return false;
                            }">delete</a>
