<div class="contentBlock">
    <div class="contentTitle">
        ...
        <? if ($User->CheckPermissions('edit') || ($User->CheckPermissions('editOwn') && $cont['author'] == $User->Id)) { ?>
            <a href="#" onclick="editContent(<?= $cont['id'] ?>);
                    return false;">
                <img src="<?= $this->RootUrl ?>template/img/edit.png" alt="Edit" title="Edit" />
            </a><? } ?>
        <? if ($User->CheckPermissions('delete') || ($User->CheckPermissions('deleteOwn') && $cont['author'] == $User->Id)) { ?>
            <a href="#" onclick="if (confirm('Are you sure?')) {
                        deleteContent(<?= $cont['id'] ?>);
                    }
                    return false;">
                <img src="<?= $this->RootUrl ?>template/img/delete.png" alt="Edit" title="Edit" />
            </a><? } ?>
    </div>
