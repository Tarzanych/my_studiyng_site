...
<div class="contentBlock">
    <div class="contentTitle">
        ...
        <? if ($User->CheckPermissions('edit') || ($User->CheckPermissions('editOwn') && $this->Content['author'] == $User->Id)) { ?><a href="#" onclick="editContent(<?= $this->Content['id'] ?>);
            return false;"><img src="<?= $this->RootUrl ?>template/img/edit.png" alt="Edit" title="Edit" /></a><? } ?>
        <? if ($User->CheckPermissions('delete') || ($User->CheckPermissions('deleteOwn') && $this->Content['author'] == $User->Id)) { ?><a href="#" onclick="if (confirm('Are you sure?')) {
                        deleteContent(<?= $this->Content['id'] ?>);
                    }
